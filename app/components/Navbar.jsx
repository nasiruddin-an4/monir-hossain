    ...
    <nav 
      className={`sticky top-0 w-full z-50 transition-all duration-500 border-b ${
        scrolled 
          ...
          : 'bg-[#0a0e27] border-transparent py-5 lg:py-7'
      }`}
    >
      {/* Main Navbar Content */}
        ...
        <div className="hidden xl:flex items-center gap-10 2xl:gap-14">
          ...
          {navbarData.mainNav.map((link) => (
            <Link 
              key={link.label}
              href={link.href}
              className={`text-xs uppercase tracking-[0.2em] font-bold transition-all duration-300 relative flex items-center gap-1 ${isActive(link.href) ? 'text-white' : 'text-slate-300 hover:text-white'} hover:scale-105 group/link`}
            >
              {link.label}
          ...
          {navbarData.dropdowns.map((dropdown) => (
            <div key={dropdown.label} className="relative group/dropdown py-4">
              <button 
                className={`text-xs uppercase tracking-[0.2em] font-bold transition-all duration-300 relative flex items-center gap-1 ${openDropdown === dropdown.label ? 'text-white' : 'text-slate-300 hover:text-white'}`}
              >
                {dropdown.label}
              ...
              <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 opacity-0 invisible group-hover/dropdown:opacity-100 group-hover/dropdown:visible transition-all duration-300 transform translate-y-4 group-hover/dropdown:translate-y-0 min-w-[260px]">
                <div className="bg-[#0f1535]/90 backdrop-blur-3xl border border-indigo-500/20 rounded-2xl p-2 shadow-2xl shadow-indigo-900/50 overflow-hidden ring-1 ring-white/10">
      ...
      {/* Mobile Menu Overlay */}
      <div 
        className={`fixed inset-0 bg-[#0a0e27]/95 backdrop-blur-2xl z-40 transition-all duration-500 xl:hidden ${
              ...
              <div key={dropdown.label} className="flex flex-col">
                ...
                <div 
                  className={`grid transition-all duration-300 ease-in-out ${
                    openDropdown === dropdown.label ? 'grid-rows-[1fr] opacity-100 mt-2' : 'grid-rows-[0fr] opacity-0'
                  }`}
                >
                    ...
                    {dropdown.items.map((item) => (
                      <Link 
                        ...
                        className={`text-sm py-2 px-3 rounded-lg transition-all ${
                          isActive(item.href) ? 'text-white bg-white/5' : 'text-slate-400 hover:text-white hover:bg-white/5'
                        }`}
                      >
                        {item.name}
                      </Link>
